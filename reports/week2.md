...
> **Center of molecule** is defined as the *<ins>midpoint between both oxygen atoms</ins> in the molecule*.
...
After a long discussion, we started with the idea to solve the blocking issue by the following algorithm:
1. Draw a line between two selected nodes.
2. Around all nodes, we define a "hitbox" by drawing a circle of radius $r$ (an assumption made to represent the size of the molecule).
3. Other than the two selected nodes, the shortest distance, $d$ between a line and an arbitrary node is found. This distance is calculated for all nodes surrounding the pair of nodes
4. If this distance $d$ is shorter than $r,$ that is ($d<r),$ then the line intersects a hitbox. Thus, the pair is blocked.

<div style="text-align: center;">
  <img src="../reports/images/02_shortest_distance_between_a_point_and_a_line.jpg" style="max-width: 100%; height: auto;">
  <figcaption>Figure 2.1 — The above illustrates the shortest distance between a line and a point.</figcaption>
</div>

The problem with this algorithm would be the large amount of computation required. To overcome this, we implemented:
- Dividing the unit cell into subboxes, such that each subboxes houses $0-2$ molecules.
- Molecules contained within the subbox, as well as within all adjacent subboxes are deemed *nearby to each other*. Thus, computation number is greatly reduced.
...
The goal this week was implement the first iteration of our idea:
- **Find the midpoints**
- **Create the subboxes**
- **Define the edges**
...
- After defining the column spacing with their names, using `pd.read_fwf()` we extract all the data.
- This leaves us with un-extracted data `title`, `number of atoms` and box `dimensions`
- To extract the rest, we used python built in function `f.readline()` twice to read the title and number of atoms.
- Afterwards we use the `seek` function and a `while` loop to move the cursor all the way to the start of the line with box dimensions. The `whence` parameter can be set to `2` so that the calculation starts at the end of the file. Otherwise, setting it to `1` makes it starts from the current cursor position.
<div style="text-align: center;">
...
- We start off by extracting the positions of two oxygen atoms, O<sub>1</sub> and O<sub>2</sub> in the molecule
- To solve the periodic boundary condition problem here, we first find the ***difference*** of `xyz` coordinates between the atoms, which *should more or less be <ins>smaller than one</ins>* $<|1|$. 
- Meanwhile, if the *__difference__ is <ins>greater than one</ins>* $ >|1|$, that indicates that O<sub>1</sub> is bigger than O<sub>2</sub>, indicating that the position of O<sub>1</sub> is larger than O<sub>2</sub>, and thus we target it to readjust it to a smaller position. We do this by subtracting O<sub>1</sub> by the `box_dimensions`.
- On the flip side, O<sub>2</sub> would be subtracted by the `box_dimensions` instead.
...
> In the end, we <u>defined the number of subboxes to be 1331</u>, which is the closest cube root to 1501, the number of molecules in the unit cell.

<div style="text-align: center;">
  <img src="../reports/code/02code3.jpg" style="max-width: 100%; height: auto;">
</div>

Afterwards, we looked to find the number of molecule in each subbox:
...
- For every selected box, we use a nested for loop to find the indices of the boxes in a 3 subbox by 3 subbox cube centered on the selected box. This cube obviously also includes the selected box so we remove it afterwards
<div style="text-align: center;">
...
- Using `numpy` `ndarray` class which allow for simpler use of mathematical operations between arrays of same size, we first find the difference of the coordinates of the two points, `p1` and `p2`
...
1. We first define functions to:
    ...
    2. Find angles from a given node to another point, using the function `np.arctan2`, which unlike `np.arctan` ranges between [$-\frac{\pi}{2}$,$\frac{\pi}{2}$], `np.arctan2` ranges between [$-\pi$,$\pi$] radians. It also correctly identifies quadrants of the angle based on the sign of the inputted y and x values.
